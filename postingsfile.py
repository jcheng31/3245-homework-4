...
###############################################################################
    ...
    def __write_DWord(self, dword):
        assert type(dword) == int
        byte_array = struct.pack(PostingsFile.DWORD_FMT, dword)
    ...
    def __read_string(self):
        ...
        result = str(self.__fd.read(length))
        return result
...
###############################################################################
    ...
    def __write_idtf_tuple(self, _tuple):
        assert len(_tuple) == 2
        cur = self.__fd.tell()
        self.__write_string(_tuple[0])
        effect = self.__write_DWord(_tuple[1])

    @requires_open
    def __read_idtf_tuple(self):
        result = (self.__read_string(), self.__read_DWord())
        return result
